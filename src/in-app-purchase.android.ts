...
export class InAppPurchaseManager extends InAppPurchaseManagerBase {
    ...
    private createInAppTransactionState(purchase: com.android.billingclient.api.Purchase): InAppPurchaseTransactionState {
        const productIdentifier = purchase.getSkus().get(0)/* API CHANGE */ || JSON.parse(purchase.getOriginalJson())['productId']
        ...
        return new Promise<InAppOrderHistoryResult>((resolve, reject) => {
            try {
                ...
                for (let i = 0; i < allPurchases.size(); i++) {
                    ...
                    const productIdentifier = (purchaseRecord.getSkus() && purchaseRecord.getSkus().get(0)) ? purchaseRecord.getSkus().get(0)/* API CHANGE */ : JSON.parse(purchaseRecord.getOriginalJson())['productId']
